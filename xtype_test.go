...
func TestNot(t *testing.T) {
	...
	var b T
	r := xtype.Not(b)
	if xtype.Bool(r) != true {
		t.Fatal("must true")
	}
	if s := reflect.TypeOf(r); s.Name() != "T" {
		t.Fatal(s)
	}
	if b != false {
		t.Fatal("must false")
...
func TestNegUint8(t *testing.T) {
	...
	if reflect.TypeOf(r).Name() != "T" {
		t.Fatal(r)
	}
	n = 1
	r = xtype.NegUint8(n)
	if xtype.Uint8(r) != 255 {
		t.Fatal(r)
	}
	if n != 1 {
		t.Fatal(n)
...
func TestMakeComplex64(t *testing.T) {
	type T complex64
	typ := xtype.TypeOf(T(1 + 2i))
	if r := xtype.MakeComplex64(typ, complex64(1+2i)); xtype.Complex64(r) != 1+2i {
		t.Fatal(r)
	}
}
...
func TestBytes(t *testing.T) {
	type T []byte
	i1 := T("hello")
	if n := xtype.Bytes(i1); string(n) != "hello" {
		t.Fatal("check", n)
	}
	typ := xtype.TypeOf(i1)
	i2 := xtype.Make(typ, []byte("world"))
	if n := xtype.Bytes(i2); string(n) != "world" {
		t.Fatal("new at", n)
	}
}
...
func TestRunes(t *testing.T) {
	type T []rune
	i1 := T("hello")
	if n := xtype.Runes(i1); string(n) != "hello" {
		t.Fatal("check", n)
	}
	typ := xtype.TypeOf(i1)
	i2 := xtype.Make(typ, []rune("world"))
	if n := xtype.Runes(i2); string(n) != "world" {
		t.Fatal("new at", n)
	}
}
...
func TestPointer(t *testing.T) {
	type T int
	v := new(T)
	if p := xtype.Pointer(v); p != unsafe.Pointer(v) {
		t.Fatal(p)
	}
}
